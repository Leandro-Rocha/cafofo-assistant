...
export const cafofoFlixWizard = new Scenes.WizardScene(
    ...
    async (ctx) => {
        ...
        await ctx.reply(`Achei esses resultados`, Markup.inlineKeyboard([...results.map((result) => Markup.button.switchToCurrentChat(`${result.title}`, `/list ${result.url}`))], { columns: 1 }))
    ...
    async (ctx) => {
        ...
        await ctx.reply(`Achei esses resultados`, Markup.inlineKeyboard([...results.map((result) => Markup.button.switchToCurrentChat(`${result.title}`, `/download ${result.url}`))], { columns: 1 }))
