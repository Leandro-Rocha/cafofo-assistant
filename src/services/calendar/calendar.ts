...
if (!process.env.CALENDAR_ID) throw new MissingEnvironmentProperty('CALENDAR_SERVICE_ADDRESS')
...
export async function listNextEvents(maxResults = 10): Promise<CalendarEvent[]> {
    const eventsCall = await axios.get(CALENDAR_SERVICE_ADDRESS + `/events/${CALENDAR_ID}`, { params: { maxResults } })
